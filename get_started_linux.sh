...
BOT_ACCOUNT_ID=$BOT_ACCOUNT_ID
AGENT_SECRET_PROJECT=$PROJECT_ID
...
# ==============================================================================
phase_12_memory_doc() {
    say "Phase 12: Persistent memory via Google Doc (optional)"
    echo "If you want your agent to remember things across conversations,"
    echo "the easiest way is to give it read/write access to a Google Doc."
    echo "Each session, it reads the doc, uses the contents in its prompt,"
    echo "and (optionally) writes back updated notes."
    echo

    if ! prompt_yn "Wire up a Google Doc for persistent memory?"; then
        ok "Skipping memory doc setup."
        hr
        return 0
    fi
  ...
  ${BOLD}Now share the Doc with your agent's service account:${NC}
  ...
  Until you do this, the agent will get a 403 when it tries to read the doc.

  The doc-reading / doc-writing functions are commented out in
  custom_functions.py. Uncomment them and add to root_agent.tools when
