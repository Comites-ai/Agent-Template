...
- Pick the model that fits the work. `HIGH_QUALITY_AGENT_MODEL` in `.env` is what `root_agent` uses (Claude via `model_utils.py`); `SPECIALIST_AGENT_MODEL` is the convention for sub-agents and is the root's backup; `SEARCH_AGENT_MODEL` (Gemini) is for sub-agents that use `google_search`; `VISION_MODEL` (Gemini) covers direct image/PDF extraction calls. Never pass a bare model string to `Agent(model=...)` — use the `model_utils` factories.
...
## Optional: joining a Magister (chief-of-staff) estate

If your deployment has a coordinating Magister agent, set
`MAGISTER_DISPLAY_NAME` in `.env` and enable the agents MCP toolset in
`agent.py`. That activates the standard capability suite
(`comites_standard.py`): this agent answers the standard inquiries
(`review_idea`, `goal_progress`, `focus_items`, `daily_metric`) and routes
shared-asset writes (todo list, calendar, tracking documents) through the
Magister instead of touching them directly. Prune `inquiries.json`'s
`"standard": true` entries to the ones that fit this agent's domain. With
`MAGISTER_DISPLAY_NAME` unset (the default) all of this stays dormant and
the agent is fully standalone.
