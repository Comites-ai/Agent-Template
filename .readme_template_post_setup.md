...
This agent was bootstrapped from the [Comites.ai Agent Template](https://github.com/Comites-ai/agent-template). The deployed Reasoning Engine currently responds with a fixed greeting (the template stub) until you replace `STUB_INSTRUCTION` in [`agent.py`](agent.py) with the real prompt.
...
The agent works end-to-end as soon as you've deployed once (it'll respond with the stub greeting on whichever platform you enabled). From there, the typical buildup is:
...
### 5. Add external API integration with secrets
...
### 6. Add image / multimodal support (optional)
