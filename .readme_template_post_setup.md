...
### 4b. Publish and consume agent-to-agent inquiries (optional)

Other agents on your Forum can exchange data with yours through the
Forum-hosted agents (A2A) MCP server:

- **Publish**: edit [`inquiries.json`](inquiries.json) with the requests
  your agent can field (name, description, request/response formats), add
  a prompt section in `agent.py` that answers each one in EXACTLY the
  published format, and redeploy — `register_agent.py` writes them to
  Firestore automatically. Inbound agent queries arrive prefixed
  `[From Agent: <Agent> | On Behalf Of: <User>]`; scope answers to that
  user (see AGENTS.md rules 13-14).
- **Consume**: uncomment the `agents_toolset` block in `agent.py` (same
  API key as the scheduler MCP) and instruct your prompt when to call
  `list_agents` / `get_agent_inquiries` / `query_agent` — always passing
  `on_behalf_of` from your `[From: <name>]` prefix.
- **Develop**: attach Claude Code to `{FORUM_URL}/api/v1/mcp/agents/`
  with your agent's key to explore and test-query attached agents.
