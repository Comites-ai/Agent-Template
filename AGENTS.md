...
**Cross-project SA usage org policy (two overrides — both required):** GCP enforces `constraints/iam.disableCrossProjectServiceAccountUsage` by default (and the default has been *enforced* on all orgs created since Sep 2024). The constraint applies on BOTH sides of cross-project SA usage:
...
So terraform provisions two overrides:

- `google_project_organization_policy.allow_cross_project_sa` — on the agent's own project (releases the per-agent SA).
- `google_project_organization_policy.allow_cross_project_sa_usage` — on the Forum's project (lets the Reasoning Engine adopt the external SA).

Skipping either one produces the same painful failure mode: `adk deploy agent_engine` creates the Reasoning Engine cleanly, and then every metadata-server token request at runtime returns 500 with no useful log line — the engine never serves a single user message. **The Forum-side override is shared infrastructure** across every agent that deploys to the Forum; first agent's terraform creates it, subsequent agents' applies are no-ops, but `terraform destroy` on any agent reverts it and breaks every other agent. If you're tearing down one agent while others still exist, comment that resource out before destroying. The agent-side override is per-agent and disappears cleanly with the agent's own project. The operator running `terraform apply` needs `roles/orgpolicy.policyAdmin` on **both** projects for both overrides to succeed.
