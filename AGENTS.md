...
**Cross-project SA usage org policy:** GCP enforces `constraints/iam.disableCrossProjectServiceAccountUsage` by default, which would block Vertex AI in the Forum project from running as a service account from another project (exactly what we do). The terraform handles this via `google_project_organization_policy.allow_cross_project_sa_usage`, which disables enforcement at the Forum's project level. The operator running `terraform apply` needs `roles/orgpolicy.policyAdmin` on the Forum project for this to succeed. **This policy is shared across every agent that deploys to the Forum** — the first agent's terraform creates it, subsequent agents' applies are no-ops, but `terraform destroy` on any agent reverts it and breaks every other agent. If you're tearing down one agent while others still exist, comment that resource out before destroying.
